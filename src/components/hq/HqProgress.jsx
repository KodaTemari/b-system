...
import {
  CM_SHOW_DELAY_MS,
  DEFAULT_HQ_TD_APPROVER_NAME,
  SHOW_MATCH_ID,
  statusClassMap,
  statusLabelMap,
} from './hqProgressConstants.js';
import { HqProgressManualPaperModal } from './HqProgressManualPaperModal.jsx';
import {
  buildCourtApproveSyntheticManualRequest,
  normalizeScoreboardGameMatchEnds,
} from './hqProgressManualPaperHelpers.js';
import {
  courtHasAnnouncedOrInProgressMatch,
  getAnnounceableMatchesForSlot,
  toHqApproveConfirmModalTitle,
  toPoolRoundLabel,
} from './hqProgressMatchContext.js';
import { ManualPaperRequestReviewDl } from './ManualPaperRequestReviewDl.jsx';
import { toClockLabel, toCurrentTimeLabel } from './hqProgressTimeLabels.js';
...
const HqProgress = () => {
  ...
  /** オペレーター: ON のとき「配信取り消し」を表示（既定 OFF） */
  const [operatorDetailView, setOperatorDetailView] = useState(false);
  const mode = useMemo(() => {
  ...
  return (
    <main className="hqProgressPage hqProgressPage--fixedHeader">
      <section className="hqProgressSection">
        ...
        {isOperatorMode ? (
          <div className="hqProgressOperatorOptionsBar">
            <div className="hqProgressOperatorOptionsBarStart">
              <span className="hqProgressDetailViewSwitchLabel" id="hq-operator-detail-label">
                詳細表示
              </span>
              <button
                type="button"
                className={`hqProgressDetailViewSwitch${operatorDetailView ? ' isOn' : ''}`}
                role="switch"
                aria-checked={operatorDetailView}
                aria-labelledby="hq-operator-detail-label"
                onClick={() => setOperatorDetailView((v) => !v)}
              >
                <span className="hqProgressDetailViewSwitchThumb" />
              </button>
            </div>
            {hqMainView === 'schedule' && operatorDetailView ? (
              <div className="hqProgressOperatorOptionsBarActions">
                <button
                  type="button"
                    ...
                    {schedule.courts.map((court) => {
                      ...
                      return (
                        <td key={`${slot}-${court}`} className="scheduleCell" style={inProgressPoolBgStyle}>
                          ...
                          {isOperatorMode &&
                            (canAnnounce || (operatorDetailView && canUnannounce)) && (
                            <div className="hqProgressButtonRow">
                              {canAnnounce ? (
                                <button
                                  type="button"
